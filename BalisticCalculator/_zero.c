...
float SightHeight, float ZeroRange, float yIntercept) {
	...
	float t = 0;
	float dt = 1 / Vi; // The solution accuracy generally doesn't suffer if its within a foot for each second of time.
	float y = -SightHeight / 12;
	float x = 0;
	float da; // The change in the bore angle used to iterate in on the correct zero angle.

	// State variables for each integration loop.
	float v = 0, vx = 0, vy = 0; // velocity
	float vx1 = 0, vy1 = 0; // Last frame's velocity, used for computing average velocity.
	float dv = 0, dvx = 0, dvy = 0; // acceleration
	float Gx = 0, Gy = 0; // Gravitational acceleration

	float angle = 0; // The actual angle of the bore.
